...
@pytest.mark.asyncio
async def test_a_task_file_seeds_the_run_without_typing(monkeypatch, tmp_path):
    """Pasting multi-line Markdown into a single-line prompt truncates it.

    prompt_toolkit submits at the first newline, so a 5.8 KB task text pasted
    into the TUI would become its first line. The file path avoids the input box.
    """
    seeded: list[str] = []
    ...
    class Runtime:
        ...
        async def start(self) -> None:
            raise AssertionError("start_task owns the fresh path")
    ...
    task.write_text("line one\n\nline two\n", encoding="utf-8")
    monkeypatch.setattr(entrypoint, "ResearchRuntime", Runtime)
    monkeypatch.setattr(entrypoint, "AthenaApp", App)
    args = entrypoint._parse(
        ["--project", str(tmp_path / "p"), "--task-file", str(task)]
    )

    assert await entrypoint._run(args) == 0
    assert seeded == ["line one\n\nline two"]


@pytest.mark.asyncio
async def test_an_empty_task_file_is_refused(monkeypatch, tmp_path):
    class _Tree:
        def best_experiment_id(self):
            return None
    ...
    task.write_text("   \n", encoding="utf-8")
    args = entrypoint._parse(["--task-file", str(task)])

    with pytest.raises(SystemExit):
        await entrypoint._run(args)
