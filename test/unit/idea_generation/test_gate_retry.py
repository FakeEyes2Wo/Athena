...
@pytest.mark.asyncio
async def test_finish_ideator_batch_passes_runtime_client_to_gate(tmp_path, monkeypatch):
    agents = _Agents()
    ...
    await runner._finish_ideator_batch(
        HypothesisBatch(hypotheses=[]), rejections=[]
    )
