...
light_hard_gate + pairwise ranking, with the LLM boundary faked out."""
...
def _draft(statement: str, *, testable: bool = True) -> IdeatorHypothesisDraft:
    return IdeatorHypothesisDraft(
        ...
        supported_premises=[
            ClaimEvidence(
                ...
                role=ClaimRole.SUPPORTED_PREMISE, supporting_refs=["eda-report"],
            )
        ],
        predicted_observations=["accuracy increases"] if testable else ["accuracy increases"],
...
_FAKE_RESPONSES = {
    FalsifiabilityJudgment: lambda: FalsifiabilityJudgment(
        testable_implication="measure validation accuracy before/after", unobservable_variables=[],
        is_falsifiable=True,
    ),
    SkepticJudgment: lambda: SkepticJudgment(
        critique="reasonable", unaddressed_risks=[], fatal_flaw_found=False,
    ),
    PairwiseJudgment: lambda: PairwiseJudgment(winner="candidate_a", rationale="clearer premise"),
}
...
@pytest.mark.asyncio
async def test_run_light_pipeline_keeps_and_ranks_passing_drafts(tmp_path):
    store = LocalArtifactStore(tmp_path / "artifacts")
    drafts = [_draft("Cabin deck predicts survival"), _draft("Ticket group size predicts survival")]

    kept = await gate_module.run_light_pipeline(drafts, model="fake-model", artifacts=store)
...
async def test_run_light_pipeline_drops_unfalsifiable_draft(tmp_path, monkeypatch):
        ...
        if schema is FalsifiabilityJudgment:
            ...
            if calls["n"] == 1:
                return FalsifiabilityJudgment(
                    testable_implication="", unobservable_variables=["true cause unknown"],
    ...
    for module_path in (
        ...
        monkeypatch.setattr(f"{module_path}.single_turn_structured_chat", _mixed_chat)
