...
async def test_evaluator_prompts_match_the_data_source(
    ...
    # PREPARE freezes SEARCH only; FINAL is lazy and must not be provisioned.
    assert len(tasks) == 1
    assert bundle.search_ref == "ref-1"
    assert bundle.final_ref is None
    assert checked == []
    if platform_split:
        assert tasks[0].startswith("build evaluator")
        assert "Do not read FINAL features or labels" in tasks[0]
        return
    assert "SEARCH partition" in tasks[0]
    assert "You are building the FINAL evaluator" not in tasks[0]
    assert "SHA-256" in tasks[0]
    assert "group-disjoint" in tasks[0]
    assert "__athena_row_id,label" in tasks[0]
    assert "contract_version=2" in tasks[0]
    assert "complete class_labels" in tasks[0]
    assert "missing, duplicate, or unexpected ids" in tasks[0]
    assert "final_labels.csv in the platform" not in tasks[0]
...
async def test_final_prompt_carries_the_frozen_search_metric(
    ...
    runtime.supervisor.evaluator_ref = "search-ref"
    _frozen_evaluator(runtime.workspaces_root / "evaluator" / "evaluate")
    monkeypatch.setattr(
        evaluator_module,
        "reusable_ref",
        lambda _runtime, ref: _return_ref(ref),
    )
    monkeypatch.setattr(evaluator_module, "run_evaluator_agent", fake_evaluator)

    async def _return_ref(ref):
        return ref

    await evaluator_module.prepare_final_evaluator(runtime, "build evaluator")

    assert "task_type=classification" in tasks[0]
    assert "primary_metric=macro_f1" in tasks[0]
    assert 'class_labels=["negative", "positive"]' in tasks[0]
