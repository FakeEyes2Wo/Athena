...
def test_nested_workspace_shell_workdir_keeps_private_directory_protected(
    tmp_path: Path,
) -> None:
    from athena.execution.runtime import _reject_athena_shell_write

    workspace = tmp_path / ".athena" / "demo" / "workspaces" / "eda"
    _reject_athena_shell_write("python analyze.py", str(workspace), workspace)
    _reject_athena_shell_write("python analyze.py", "reports", workspace)
    with pytest.raises(ValueError, match="read-only"):
        _reject_athena_shell_write("python analyze.py", ".athena", workspace)
    with pytest.raises(ValueError, match="read-only"):
        _reject_athena_shell_write(
            "Set-Content .athena/state.json '{}'", None, workspace
        )
