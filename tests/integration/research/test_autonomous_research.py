...
async def test_default_validation_adapter_uses_frozen_inputs_and_supervisor_checkpoint(
    ...
    captured = {}
    final_prepare_calls: list[str] = []

    async def prepare_final(_runtime, task):
        final_prepare_calls.append(task)
        return "lazy-final-evaluator"
    ...
    monkeypatch.setattr(
        "athena.research.runtime.phase_runner.prepare_final_evaluator",
        prepare_final,
        raising=False,
    )
    runtime = ResearchRuntime(
    ...
    assert validation_input.final_evaluator_ref == "lazy-final-evaluator"
    assert final_prepare_calls == [runtime.task_text]
