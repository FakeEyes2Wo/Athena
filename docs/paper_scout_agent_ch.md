...
**这三档里只有"只要 3 分"那一档可以跨模型使用。** 同一批 150 篇论文让两个模型各打一遍：

| 门槛 | `qwen3.7-plus` | `qwen3.6-flash` |
| --- | --- | --- |
| `> 0.45`（只要 3 分） | 2 篇 | 2 篇，且是同样那 2 篇 |
| `> 0.2`（含 2 分） | 29 / 37 篇 | **17 篇** |

3 分是"直接回答查询"、1 分是"沾边"，判据都清晰；2 分是"切题但不完全满足"，边界靠临场判断。
后果是 **2 分档连同一个模型自己连打两遍都只有 77% 稳定**（27 篇 vs 35 篇），而整体档位一致率
是 94%。换模型后差一倍。

所以配 `ATHENA_SCORER_MODEL` 时的纪律只有一条：`retain_threshold` 保持默认 0 或用 0.5，
**用 0.3 就必须在换模型后重测**。
...
打分是四档离散的，所以交付名额几乎总是在某一档**内部**被截断。实测某轮：8 篇满分直接入选，
剩下 5 个名额由 **35 篇同为 0.45 的论文**争夺——将近一半的交付集合由同分次序决定。

此前 `ranked()` 用 `paper_key` 字典序，那不是排序设计而是字符串比较的副产物，两条系统性偏置：

- `arxiv:` 恒排在 `doi:` 之前 → 那 35 篇里的 **23 篇期刊论文一篇都进不去**；
- arXiv 编号升序等于最老的优先 → 选出来的 5 篇全在 2012–2020，而该档中位年份是 2023。

**换成年份降序试过，不成立。** 选出来的 5 篇全是 2026 年的边缘论文（多模态生物医学融合网络、
蛋白质数据增强），反而不如原来选中的 DART 与 Imbalance-XGBoost 贴题。同一档内部按定义没有
相关性差异，任何单调偏好都只是换一个方向的臆断。

`tie_break()` 因此只做一件事：把字典序换成 `paper_key` 的稳定散列。次序仍然完全可复现，但不再
与通道、年代或编号相关。三轮真实池子重放，交付集合各变动 1–3 篇。

**它保证的是"不系统性排除"，不是"按比例代表"。** 单轮仍是小样本抽取——E 轮那 5 个名额从 12
篇 arXiv / 23 篇期刊里抽，抽到 4 篇 arXiv，比期望偏高。要真正区分同档论文，得让打分器给出更细
的分数，而不是在这一层发明信号。
