...
- athena：`.athena/state.json` + `research_tree.json` + artifacts + `.athena/autoresearch/hypothesis_pool.json`（可选）
- external：用户提供的 `evidence_chain.json`

## 步骤

1. athena / none 引擎：跑 `extract --adapter athena`，输出 `<run>/records-paper/<run_id>/evidence_chain.json`。
2. external 引擎：直接确认用户提供的 evidence_chain schema 合法，不重新抽取。
3. `candidate_id` 注入：athena adapter 读 `hypothesis_pool.json` 的 `origin_candidate_id` 映射填 `experiments[].candidate_id`；无映射置 null。
