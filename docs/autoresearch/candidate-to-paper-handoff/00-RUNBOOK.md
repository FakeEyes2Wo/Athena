...
node ... run-candidate-to-paper.mjs \
  ...
  [--work-dir .] \
  [--mode auto|semi-auto] \
  [--engine auto|athena|external|none] \
...
| 3 | ideation | 03 | IDEAS/*.md + hypotheses 入树/池 | 门禁 PASS/EXPLORATORY ≥1，否则重提 ≤2 |
| 4 | experiment_plan | 04 | EXPERIMENT_PLAN.md（预注册） | plan 覆盖所有 QUEUED hypothesis |
| 5 | experiment | 05 | ResearchTree + 实验产物 | 见 05 |
...
- 05 实验结算后写 tree；06 的 evidence_chain 行带 `candidate_id`。
...
| `max_experiments` | experiment | 触发 search_limit / VALIDATE 决策 |
...
- 崩溃恢复：读 AutoResearch `state.json` 的当前 step 续跑；experiment 用
  `FixedFlowSupervisor.recover()`；evidence 源哈希未变则跳过重抽取。
