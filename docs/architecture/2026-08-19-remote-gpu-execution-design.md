...
### 11.8 传输流水线：瓶颈是往返次数，不是带宽（2026-08-20）

**量出来的诊断。** 分块上传原本一块一等，于是吞吐 = 块大小 ÷ 往返时延，与带宽
无关。同一条链路上的三个数：

| | 实测 | 受什么限制 |
|---|---|---|
| Athena 推 | 1.9 MiB/s | 协议：64 KiB ÷ 31 ms ≈ 2 MiB/s，几乎完全吻合 |
| 裸 `scp` 推 | 5.9 MiB/s | 笔记本的上行链路上限 |
| GPU 机自己拉 | 14.5 MiB/s | 远端出口带宽（清华镜像，200 MiB / 13.8 s） |

**改了三处，全是"别一个一个等"：**

1. **块级窗口**（`TRANSFER_WINDOW = 16`）。`send` 拆出来只发不等，`_write_blocks`
   按窗口收。顺序不受影响——`_send` 串行写，远端读循环顺序派发，所以同一个文件
   的块到达次序与发出次序一致，`append` 语义仍然成立。
2. **文件级并发**（`TRANSFER_FILES = 4`）。50 个小文件原本 50 次往返。
3. **流式读盘**（`send_file`）。原来 `write_file(bytes)` 要求整个文件先进内存——
   数据集单个文件几十 GB 时这是致命的，而控制节点是笔记本。读盘放线程里做。

**结果，以及为什么到此为止：**

| | 改前 | 改后 |
|---|---|---|
| 8 MiB 单文件 | 4.3 s（1.8 MiB/s） | 0.6 s（12.8 MiB/s） |
| 64 MiB 持续 | 1.9 MiB/s | **4.3 MiB/s** |
| 50 个小文件 | 1.69 s | 0.47 s |

持续吞吐 4.3 MiB/s，同一时刻裸 `scp` 是 5.9——**比值 73%，而 base64 的膨胀正好
是 1/1.33 = 75%**。也就是说现在是链路受限，剩下的差距全是 base64 那 33%。

再往下只剩一条路：换二进制分帧，去掉 base64。**不做**——那要推翻"一行一条 JSON"
这条整个协议赖以简单的性质，换来 27%；而远端直取是 14.5 MiB/s 起步，且把控制节点
从关键路径上拿掉。下一步应该是那个，不是继续榨这条管子。

五条回退即红的用例：窗口（退成一块一等则红）、文件并发（退成串行则红）、
块序（并行不能打乱 `append`）、两个调用点的流式（退成整读则红）。断言不用计时
（CI 上会抖），而是把 `put` 的确认掐掉，数"不等确认能发出多少"。
...
- **大数据集**：真机上量到 128 MiB（见 11.8），50 GiB 按当前速率约 3.3 小时。
  断点续传与内容校验的逻辑覆盖是够的，但"控制节点推到一半掉线会怎样"没有量过。
- **共享数据存储**：现在只有"控制节点推"这一条路。数据若有上游（对象存储、比赛、
  一个 URL），让 GPU 机自己去拉快 3 倍以上，且控制节点不必在线。这条比继续优化
  推送更值得做，见 11.8 末尾。
