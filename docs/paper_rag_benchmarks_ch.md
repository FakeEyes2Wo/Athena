...
已落地（整改 Phase 3，实测数字见 [文献链路整改](survey_overhaul_ch.md)）：

- ~~**overview 摘要跳过前置区块**~~ —— 改的是**锚点选择**而不是摘要截取：优先 abstract，
  否则在前 6 个单元里取第一个有实质正文的。门面几乎无正文的篇数 5 → 0，不再有锚点落在
  表格或插图上。
- ~~**向量 artifact 走 mmap**~~ —— 实测峰值常驻 519 MB → 367 MB，装载 0.413s → 0.247s。
- ~~**缓存查询向量**~~ —— 挂在 `CorpusCache` 上，按编码器分桶。

待办：

1. **关键词打分引入 IDF** —— 单独看能把 MRR 抬到 0.530，但会压掉主题词本身就是泛词的
   查询（第 4.4 节）。需要更细的启用条件，不是一行权重。
2. **重新审视反面证据的取证路径** —— Ablation 在真实调研语料上覆盖 0 篇（第六节）。
3. **真实引用图** —— S2/OpenAlex 在检索阶段返回过 references，现在被丢掉，语料内引用边
   只有 16 条（每篇 0.36），两个图遍历算子因此形同虚设。

## 复现

> **这些测量已经进仓库了。** 本文原先在这里写着"基准脚本不在仓库内（属于一次性测量）"
> ——那正是整改要修的第一件事：脚本不在仓库里，任何改动都无法证明变好，任何回归也不会被
> 发现。现在三条命令随时可重跑，见 [文献链路整改](survey_overhaul_ch.md) 第一节。
>
> ```bash
> Athena-cli bench retrieval --corpus <ref>   # 逐通道 hit@k / MRR
> Athena-cli bench health    --corpus <ref>   # 摘要覆盖、章节覆盖、引用边密度
