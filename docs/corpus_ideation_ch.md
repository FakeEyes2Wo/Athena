...
**但这个 A/B 答不了原问题。** 两臂的基线差了 **0.173**（0.8637 对 0.6906），而 SEARCH
带来的改进只有 +0.050 / +0.0255——**run-to-run 的基线方差是待测效应的 3 到 7 倍**。同一
任务、同一数据、同一留出集，PREPARE 每次自己写基线模型，写出来的强弱就能差这么多。N=1
的两臂无论如何都测不出文献接地的效果。
