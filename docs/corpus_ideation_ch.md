...
- [文献调研的交付与边界](survey_limits_ch.md) — 代价、A/B 结论与全部限制
...
| 第六节"这个 A/B 答不了原问题" | 加了 `--fork-from`，但它当时跑不通，见下方修正 |
| 全文"只有 Ideator 能查语料" | PlanAgent 也拿到只读算子，且它的会话不进引用核验的账本 |

第六节那句方法论教训仍然成立，而且现在有工具支撑：**凡是要拿来下结论的分数，都该独立
重算一遍再信。**

## 九、上表第三行是错的（2026-08-18 修正）

"`--fork-from` 让两臂共用 evaluator 与基线"这句话当时不成立：**`fork_project` 在真项目上
从未成功过一次**。它按 `experiment["kind"]` 找基线，而 `Experiment` 模型没有这个字段——
`kind` 在 `plan` 里、`id` 是映射的键。对着一个刚跑完 PREPARE 的项目分叉，得到的是
`source project has no baseline experiment`。另有一处：分叉不带项目根下的 `workspaces/eda`，
于是每个 Ideator lane 都抛 "EDA workspace is stale"，异常被吞成 error 输出，表现只是
"这一臂没有假设"。

两处均已修复（`18f4d5e`、`047f379`），各带回归用例。

**最值得记的是它为什么躲了这么久。** 六点九专门为"会静默失败的那种"写了一条集成用例：
构造一个"PREPARE 已完成"的项目、分叉成两臂、断言两臂的
`(evaluator_ref, phase, corpus_ref, sota)` 逐字段相同。它一直是绿的——因为那条用例的
fixture 也按错误的形状造数据，而且从不创建 EDA 目录。

**为"这类失败很安静"而写的测试，本身可以同样安静地失效。** 判据只有一条：fixture 必须来自
产品真正写出来的东西。把 fixture 换成 `ResearchTree.to_dict` 的真实形状之后，15 条用例当场
红了 11 条。

修好之后 A/B 第一次跑通了，结论与全部限制见
[文献调研的交付与边界](survey_limits_ch.md)。一句话版本：在实测的那个任务上，开调研没有让
假设变好，两个可机械测量的指标显著变差（每轮存活假设 5–6 → 3–4，p=0.011；轮内重复度
0.192 → 0.406，p=0.014）。
