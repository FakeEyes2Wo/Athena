...
请求、状态、报告、四个阶段和顶层推进统一位于 `survey/pipeline.py`；embedding
与视觉模型实现位于 `survey/providers.py`，`wiring.py` 负责依赖装配和 Agent 工具边界。
...
在补上组合根之前，症状很具体：问模型有哪些工具，列表里没有 `paper_markdown`、`paper_fetch`
这些——不是界面的 bug，是没人注册它们。`build_corpus_index` 只被测试调用过，`paper_markdown`
与 `paper_rag` 从未在测试之外运行过。

## 三个入口

| 入口 | 给谁 | 怎么用 |
| --- | --- | --- |
| `run_survey(stack, request)` | 库调用 | 直接拿 `SurveyReport` |
| `paper_survey` 工具 | Agent | 由 `build_survey_tools(stack)` 注册，返回 `corpus_ref` |
| `Athena-cli survey` | 人 | 打印成本账，可 `--out` 落 JSON |

三者共用 `build_survey_stack()` 这一个组合根，没有第二处读环境变量的地方。

`paper_survey` 标了 `concurrency_safe=False`：一次调用会跑满三个限速上游、下载几十兆字节、
并发调用视觉与编码模型，同一 turn 里并行两次只会互相抢限流配额。已经拿到 `corpus_ref` 的
Agent 用 `build_survey_tools(stack, include_survey=False)`——把一个几分钟起步的工具摆在那里，
模型迟早会去按它。
...
LLM 凭据不在这里读：客户端一律由 `athena.core.agent.settings` 提供（`DEEPSEEK_API_KEY` /
`OPENAI_API_KEY`、`BASE_URL`、`MODEL_NAME`），与 supervisor、TUI 共用同一份 `.env` 契约。
survey 只额外读下面这些。

| 变量 | 用途 | 缺失时 |
| --- | --- | --- |
| `ATHENA_SURVEY_MODEL` | PaperScout 的策略模型 | 回落 `settings.model_name()` |
...
难行。`Athena-cli survey --check` 只装配并报告能力，不发起任何检索。
...
```bash
Athena-cli survey --check                        # 只装配自检
Athena-cli survey --query "..."                  # 全链路
Athena-cli survey --papers 1706.03762,1512.03385 # 跳过检索，单独量下游
```
