...
Preflight evidence (2026-09-02): existing unrelated changes were
`M docs/superpowers/plans/2026-09-02-jw-ssd-tui-run.md` and
`?? .dsh_plugins/`; the index was clean. The requested baseline slice passed
`96 passed in 76.14s`. Work continued in the shared working tree, so no isolated
...
### Goal extension: baseline simplification and durable experiment reports

- [x] Split baseline evidence research from trusted baseline execution.
- [x] Prevent production PREPARE overrides from bypassing the evidence gate.
- [x] Validate reusable evaluator bundles and SEARCH/FINAL isolation on every resume.
- [x] Freeze evaluator contract v2 with one prediction identity, primary metric, and
  complete classification label universe across SEARCH and FINAL.
- [x] Persist every settled baseline/SEARCH/FINAL run with its result, artifacts, and
  cause under `.athena/exp_docs/runs/`.
- [x] Regenerate `FINAL_REPORT.md` and `OPTIMIZATION.md` from the durable tree after
  every settlement, including failed-run diagnostics and direction-aware deltas.
- [ ] Verify the complete research suite and the resumed JW-SSD TUI run.

### Task 7: Documentation, full verification, and plan closeout

#### Task 7A: Restore writable PREPARE workspaces for named GUI sessions

**Files:**
- Modify: `src/athena/research/runtime/bootstrap.py`
- Modify: `src/gui_gateway/handler.py`
- Create: `test/unit/research/test_runtime_workspace_paths.py`
- Modify: `tests/test_gui_gateway_handler.py`

**Root-cause contract:** named-session state roots remain protected under
`.athena/conversations/<session-id>`, while their agent worktrees live at
`workspaces/conversations/<session-id>`. Do not relax the `.athena` file-tool guard or
the authoritative EDA/baseline gate.

- [x] **Step 7A.1: Add failing workspace-layout and cleanup regression tests**
- [x] **Step 7A.2: Route named-session worktrees outside `.athena`**
- [x] **Step 7A.3: Delete the named-session worktree root with session state**
- [x] **Step 7A.4: Verify focused agent-tool, runtime-path, GUI-session, PREPARE, and baseline-gate tests**

Task 7A verification evidence (2026-09-03): the focused slice passed `73 passed in
36.78s`; Black and task-owned `git diff --check` passed. The broader research suite
completed with `811 passed, 3 failed` in `2056.05s`; all three failures are the existing
20-second startup timeout in `test_rolling_search.py` and reproduce when run alone.
They do not exercise named-session path construction or GUI session cleanup.

#### Task 7B: Recover the native directory dialog from stale start paths

**Files:**
- Create: `athena-gui/src-tauri/src/commands/dialog.rs`
- Modify: `athena-gui/src-tauri/src/commands/mod.rs`
- Modify: `athena-gui/src-tauri/src/lib.rs`
- Modify: `athena-gui/src/lib/workspaceDialog.ts`
- Modify: `athena-gui/src/lib/__tests__/workspaceDialog.test.ts`

**Interfaces:**
- Produces: Rust command
  `workspace_dialog_start_directory(app: AppHandle, requested: Option<String>) -> Option<String>`.
- Consumes: requested current workspace, Tauri home directory, and process current
  directory, in that order.
- Preserves: browser-mode no-op, native cancellation semantics, and existing picker
  error propagation.

- [x] **Step 7B.1: Write failing frontend resolver-boundary tests**

Mock `invoke` from `@tauri-apps/api/core`, then require the dialog to consume the
trusted resolver result instead of the unchecked requested path:

```ts
mocks.invoke.mockResolvedValue("C:/safe");
mocks.open.mockResolvedValue("C:/selected");

await expect(selectWorkspaceDirectory("C:/deleted")).resolves.toBe("C:/selected");
...
Also assert browser mode invokes neither `invoke` nor `open`.

- [x] **Step 7B.2: Run the focused Vitest file and confirm RED**

Run:

```powershell
npm test -- --run src/lib/__tests__/workspaceDialog.test.ts
```

Expected: failure because `workspaceDialog.ts` does not invoke
`workspace_dialog_start_directory` and still passes `C:/deleted` directly.

- [x] **Step 7B.3: Write failing Rust fallback-order tests**

Add a private `first_existing_directory` helper and unit tests using isolated real
temporary directories. Require requested directory to win; require a missing path and
a regular file to be skipped in favor of home; and require current directory to be
used when requested and home are missing.

- [x] **Step 7B.4: Run Rust tests and confirm RED**

Run:

```powershell
cargo test workspace_dialog --manifest-path athena-gui/src-tauri/Cargo.toml
```

Expected: compile failure because `commands::dialog` and its resolver do not exist.

- [x] **Step 7B.5: Implement the trusted Rust resolver and register its command**

Implement the command without a new plugin or capability:
...
Export `commands::dialog` and add the command to `tauri::generate_handler!`.

- [x] **Step 7B.6: Make the frontend dialog use only the resolved path**
...
Pass `defaultPath` to `open` only when `safeDefaultPath` is non-null. Do not call the
resolver in browser mode.

- [x] **Step 7B.7: Verify focused and complete GUI behavior**
...
git diff --check -- athena-gui/src/lib/workspaceDialog.ts athena-gui/src/lib/__tests__/workspaceDialog.test.ts athena-gui/src-tauri/src/commands/dialog.rs athena-gui/src-tauri/src/commands/mod.rs athena-gui/src-tauri/src/lib.rs
```

Expected: every command exits zero; browser mode remains inert, cancellation stays a
non-error, and no filesystem plugin or capability is added.

Verification evidence (2026-09-03): the focused frontend file passed `4/4`; the
complete GUI suite passed `142/142` across 18 files; `npm run build` exited zero;
the focused Rust resolver suite passed `3/3`; the complete Rust suite passed `10/10`
plus zero doc tests; and `cargo check` exited zero with six pre-existing
`clarification.rs` dead-code warnings. Targeted `rustfmt --check` and
`git diff --check` both exited zero. The frontend tests also confirm that browser
mode invokes neither native command nor dialog, cancellation remains `null`, and an
unusable resolver result omits `defaultPath`. No plugin, dependency, or capability
file changed.
...
- [x] **Step 1: Update user-facing workflow documentation**
