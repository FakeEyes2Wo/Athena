...
2. 本设计只完成 fixture、Rust crate 和兼容性验证，不修改真实生产调用入口。
3. 真实调用方接入与默认引擎切换必须在后续独立计划中实施。
4. 后续接入层必须保留显式 `python`/`rust` 引擎选择。
5. Rust 失败时切回 Python，不改变 Python rollout 和协议数据。
6. 只有在协议、行为、恢复和关闭测试全部通过后才允许后续计划切换默认引擎。
