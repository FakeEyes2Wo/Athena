...
- 公式密集的论文暴露出上游的更大问题：math.AP 那篇一篇就切出 3399 个句子（其余六篇合
  计 3263），被降权的 2088 句几乎全是 `\begin{aligned}`、`\Lambda_0`、`\frac{...}` 这
  类原始 LaTeX，以及被展示公式截断的 `Therefore`、`set.` 之类残句。降权只是让它们不参
  与语义排序，真正的修复在 `paper_markdown` 对数学环境的处理。
