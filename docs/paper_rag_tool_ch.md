...
- **展示公式会被逐行切碎**，这是本模块自己的缺陷，不是上游的。`split_sentences` 先按物
  理行切分——对表格行是对的（一行一条事实），对跨行的展示公式是错的：一个
  `$$ ... $$` 块会变成 `$$`、`\begin{aligned}`、`\mathcal F_{n,m,k}`、`&=`、`\frac{...}`
  等十几个"句子"。math.AP 那篇 124,315 字符的 Markdown 有 101 个 `$$` 块、861 行（占
  非空行 22%）落在块内，另有若干 `\[ ... \]` 块，最终一篇切出 3399 句，超过其余六篇之
  和。后果有三：语料膨胀、每个碎片各占一条向量因而按量计费的编码成本被放大、以及整条
  公式本身反而不再是一个可检索单元。自足度权重只是掩盖了症状——碎片不再赢，但钱照付、
  公式照样检索不到。修复应当是把展示公式块整体作为一个单元。
