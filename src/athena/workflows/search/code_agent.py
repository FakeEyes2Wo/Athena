...
class CodeAgent:
    ...
    def _generation_prompt(
        ...
        return (
            ...
            ".athena/phase_manifest.json for the frozen dataset paths.\n\n"
            "run_experiment.py must be phase-agnostic. It always: (1) trains "
            "on the manifest train path; (2) predicts on the manifest predict "
            "path; (3) writes predictions.csv with exactly two columns: "
            "__athena_row_id and prediction. Athena sets the manifest predict "
            "path to the validation features for baseline, search, and ablation "
            "experiments and to the final-test features for the frozen "
            "final-test, and sets ATHENA_PHASE accordingly. Never hardcode a "
            "phase check or refuse to run for any phase value; the same script "
            "must work unchanged for validation and final-test.\n\n"
            "Do NOT write predictions.csv, labels.csv, eval_result.json, or any "
            "runtime output file yourself during code generation; Athena "
            "produces those when it executes your script. Never create or write "
            "labels.csv. Do not run run_experiment.py or eval.py.\n\n"
