...
def start_survey(runtime: Any) -> None:
    ...
    if runtime.state.corpus_ref is not None:
        return
    runtime.session.survey.task = asyncio.create_task(runtime.run_survey())
...
async def run_survey(runtime: Any) -> None:
    ...
    try:
        ...
        async def emit(kind: str, ref: str, data: dict[str, Any] | None = None) -> None:
            """Forward one survey-agent event through the runtime projector."""
...
def ensure_survey_stack(runtime: Any) -> SurveyStack:
    ...
    if runtime.session.survey.stack is None:
        runtime.session.survey.stack = build_survey_stack(
            artifacts=runtime.store, client=runtime.client
        )
    return runtime.session.survey.stack
