...
class KnownItemQuery(BaseModel):
    ...
    rationale: str = Field(
        default="",
        description="Why these papers and no others; read when a gold moves.",
    )
...
class ChannelScore(BaseModel):
    ...
    mrr: float = Field(
        ge=0.0, le=1.0, description="Mean reciprocal rank over `scored`."
    )
...
class PaperHealth(BaseModel):
    ...
    outbound_citations: int = Field(
        ge=0, description="In-corpus papers this one cites."
    )
...
class CorpusHealthReport(BaseModel):
    ...
    citation_density: float = Field(
        ge=0.0,
        description="Citation edges per paper; near zero makes paper_cites inert.",
    )
    visual_links: int = Field(ge=0)
