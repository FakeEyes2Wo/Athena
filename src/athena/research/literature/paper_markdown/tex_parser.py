...
from athena.research.literature.paper_markdown.tex_tables import (
...
class TexPaperParser:
    ...
    def __init__(self, source: ExpandedTex) -> None:
        ...
        self.session = ParseSession(
            newlines=[index for index, char in enumerate(source.text) if char == "\n"],
    ...
    def add_external_bibliography(self) -> None:
        ...
        if (
            self.session.bibliography_added
            or self.session.external_bibliography is None
        ):
            return
    ...
    def parse_nodes(self, nodes: list[LatexNode]) -> None:
        ...
        def flush() -> None:
            """Commit buffered paragraph nodes as parsed elements."""
            if not buffer:
                return
        ...
        for node in nodes:
            if isinstance(node, LatexMacroNode) and node.macroname in SECTION_LEVELS:
                flush()
                level = min(
                    5, SECTION_LEVELS[node.macroname] + int(self.session.in_appendix)
                )
