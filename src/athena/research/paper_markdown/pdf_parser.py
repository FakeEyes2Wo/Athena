...
from athena.research.paper_markdown.visuals import render_page_region
...
def heading_level(
    ...
    if (
        ...
        return None
    distinct = sorted(set(round(value, 1) for value in heading_sizes), reverse=True)
    try:
...
def front_matter_boilerplate(text: str) -> bool:
    ...
    return normalized.startswith("arxiv:") or normalized.startswith("preprint")
...
def display_reference_keys(text: str) -> list[str]:
    """提取 Figure/Table 显示引用，并展开 ``Table 18, 19 and 20``。"""
    keys: list[str] = []
