"""Deterministic precedence, capability validation, retry, and safe failure."""
...
def normalize_metric_name(value: str) -> str:
    """Normalize spelling only; this function never selects a metric."""
...
class MetricCapability:
    """One metric Athena's generated evaluator is expected to implement."""
...
class MetricCapabilityRegistry:
    """Capability menu used for validation, never for scientific selection."""
    ...
    def resolve(self, name: str) -> MetricCapability:
        """Return canonical capability or fail without substituting accuracy."""
    ...
    def as_context(self) -> dict[str, MetricDirection]:
        """Return canonical metric names and directions for an LLM prompt."""
...
def assemble_evaluation_policy(
    ...
    *,
    ...
    """Apply source precedence and validate the selected metric capability."""
    ...
    if locked is None:
        ...
        metric, direction = _validated_choice(locked, active_registry)
...
async def generate_evaluation_policy(
    ...
    *,
    ...
    """Retry invalid LLM output; preserve an explicit primary on enrichment failure."""
    if max_attempts < 1:
    ...
    if locked is not None:
        ...
        return EvaluationPolicy(
            ...
            confidence=0.0,
            explanation=(
                "LLM rubric enrichment was unavailable; Athena preserved the "
                f"locked {locked.source} primary metric without inventing AI advice."
            ),
