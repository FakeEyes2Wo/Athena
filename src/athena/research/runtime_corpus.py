...
from athena.research.survey import build_survey_tools
...
def corpus_tools(runtime: Any, *, for_ideation: bool = False) -> ToolRegistry | None:
    ...
    if runtime.survey_corpus_ref() is None:
        return None
    session = RetrievalSession(runtime._ensure_survey_stack().corpus_cache)
    if for_ideation:
        ...
        runtime._corpus_sessions.append(session)
    return build_survey_tools(
        runtime._ensure_survey_stack(),
...
async def corpus_passages_read(runtime: Any) -> dict[str, list[str]]:
    ...
    if not chunk_ids:
        return {}
    stack = runtime._ensure_survey_stack()
    corpus = await stack.corpus_cache.load(runtime._store, corpus_ref)
