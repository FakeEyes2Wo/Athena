...
class IdeatorHypothesisDraft(BaseModel):
    ...
    Narrower than HypothesisDraft: no generation_strategy/sampling_probability fields —
    this Ideator explores one real EDA workspace rather than running N parallel labeled
    strategies, so those fields would be meaningless for it; code assigns fixed values when
    building the HypothesisPackage (see gate.build_package_from_ideator_draft).
...
class HypothesisPackage(BaseModel):
    ...
    sources: list[str] = Field(
        ...
        description="Paper keys the generator actually read while producing this candidate. "
                    "Handed down to the adversarial audit stages so they spend their retrieval "
                    "budget on what the generator missed instead of rediscovering what it found; "
