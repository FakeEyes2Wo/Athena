...
from athena.research.idea_generation.gatekeeper import max_total_risks, perspective_ok
...
single_turn_chat。**无经验依据**（MAX_TOLERATED_RISKS 已于 2026-08-15 按实测校准，本值尚未），属"先取保守
起点、真实跑过几轮后一并校准"。"""
...
async def revise_candidate(
    ...
    *,
    ...
    async with limited_by(llm_sem):
        draft = await single_turn_structured_chat(prompt, RevisionDraft, model=model, artifacts=artifacts)
    revised = HypothesisPackage(
        ...
        lineage_op="revise",
        # 修订改的是假设内容，不改"生成侧读过哪些论文"这一既成事实：原样搬运，
        # 否则终局刷新时 novelty 会因为 sources 变空而丢掉接力上下文、白重检索一遍。
...
def blocked_item_cleared(blocking_factor: str, reviews: list[SkepticReport]) -> bool:
    ...
    if blocking_factor == RISK_TOTAL_ITEM:
        ...
        return total <= max_total_risks(len(reviews))
    ...
    for round_index in range(1, MAX_DEBATE_ROUNDS + 1):
        ...
        try:
            revised, draft = await revise_candidate(
                ...
                artifacts=artifacts, llm_sem=llm_sem, model=model,
            )
        ...
        try:
            async with limited_by(llm_sem):
                judgment = await single_turn_structured_chat(prompt, SkepticJudgment, model=model, artifacts=artifacts)
