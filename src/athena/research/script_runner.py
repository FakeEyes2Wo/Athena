...
def _with_frozen_header(content: bytes) -> bytes:
    """Add the marker after a UTF-8 BOM and never duplicate it."""
    bom = _UTF8_BOM if content.startswith(_UTF8_BOM) else b""
    body = content[len(bom) :]
    header = _FROZEN_HEADER.encode("utf-8")
    return content if body.startswith(header) else bom + header + body
...
class DataScriptRunner:
    ...
    async def freeze(
        ...
        for path in sorted(ws.rglob("*")):
            ...
            if rel == entrypoint:
                content = _with_frozen_header(content)
        ...
        source_ref = await self._store.put_bytes(
            _with_frozen_header((ws / entrypoint).read_bytes())
        )
