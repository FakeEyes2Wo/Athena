...
class ScriptCommandError(subprocess.CalledProcessError):
    ...
    def __str__(self) -> str:
        ...
        if len(detail) > _STDERR_BUDGET:
            # 头尾都留：``uv run`` 先把进度写进 stderr（VIRTUAL_ENV 不匹配警告、
            # Using CPython、Creating virtual environment、hardlink 回退警告、
            # Installed N packages），实测 500-800 字符，traceback 排在其后。
            # 只取头部时 Agent 收到的反馈正好停在 "Installed"——2026-09-03 的
            # TESS 轮里评估器因此连拒 10 次盲改，直到轮次预算耗尽。
            head = _STDERR_BUDGET // 3
            tail = _STDERR_BUDGET - head
            detail = f"{detail[:head]} ...(略)... {detail[-tail:]}"
