...
def _run_cmd(cmd: list[str], *, cwd: Path) -> None:
    """在给定目录运行命令；失败抛 ``SubprocessError``，超时抛 ``TimeoutExpired``。

    超时用 ``TimeoutExpired``（``SubprocessError`` 子类）而非 ``RuntimeError``，
    这样上层 PREPARE 重试元组能按「可重试」捕获，而不是误判成终态。

    例：``_run_cmd(["uv", "lock"], cwd=ws)`` → 成功返回 None；失败抛
    ``SubprocessError("uv lock failed (exit 1): <stderr>")``。
    """
    _run_cmd_capture(cmd, cwd=cwd)


def _run_cmd_capture(cmd: list[str], *, cwd: Path) -> str:
    """在给定目录运行命令并返回 stdout（去空白）；超时抛 ``TimeoutExpired``。

    失败时把 stderr 带进异常消息：``CalledProcessError`` 只报命令与退出码，
    PREPARE 把它转成给 agent 的反馈后就成了「exit status 1」这种无信息错误，
    agent 无从自修，只能把 12 轮预算耗在同一处，最终进不了 SEARCH。
    """
    ...
    if result.returncode != 0:
        ...
        raise subprocess.SubprocessError(
            f"{' '.join(cmd)} failed (exit {result.returncode}): {detail[:1000]}"
        )
