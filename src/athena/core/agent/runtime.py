...
# 围栏剥离搬到了 ``athena.core.fenced_json``：2026-08-31 的 VALIDATE 崩溃说明
...
def _parse_structured_output(output_type: type[BaseModel], text: str) -> BaseModel:
    """先按原样校验，再从正文里找回**唯一**一个合法 JSON 对象。

    围栏之外模型还会在正文里夹 JSON。按位置猜（第一个 ```json 块、或结尾那个对象）
    会在正文本身含花括号时挑错东西，把一个清晰的解析错误变成一个安静的错值。这里
    改成按 schema 有效性挑：扫出全文所有能通过 schema 的对象，恰好一个才接受，多于
    一个报歧义，一个都没有就抛回原始的严格错误。
    """
    try:
