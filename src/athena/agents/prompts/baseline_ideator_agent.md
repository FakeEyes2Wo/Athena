...
units (the effective training units), labels, balance, input scale, grouping,
leakage-safe split, and source
expectations.

Use `web_search` for at least two distinct, task- and modality-specific search
queries, then use `web_fetch` to inspect the strongest sources. Prefer primary papers,
official project pages, official implementations, and official repositories.
...
yourself. Do not execute, clone, install, import, or copy candidate repositories or
