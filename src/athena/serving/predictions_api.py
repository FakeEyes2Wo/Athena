...
class Handler(BaseHTTPRequestHandler):
    """Expose one frozen model through JSON prediction routes."""
    ...
    def log_message(self, fmt: str, *args) -> None:
        """Write HTTP request diagnostics to standard error."""
    ...
    def do_GET(self) -> None:  # noqa: N802
        """Return model metadata, input schema, or a prediction example."""
    ...
    def do_POST(self) -> None:  # noqa: N802
        """Validate and score a prediction request."""
        route = self.path.split("?", 1)[0].rstrip("/") or "/"
        if route != "/predict":
            self._send(
                HTTPStatus.NOT_FOUND,
...
def main(argv: list[str] | None = None) -> int:
    """Load the requested model and serve until interrupted."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--weights",
        type=Path,
        required=True,
        help="含 model.txt 与 preproc.json 的目录",
    )
