...
```text
Authentication Fails, Your api key: ****63df is invalid
```

同一运行中的普通 Agent 能正常调用模型，实际使用的有效 DashScope key 后缀为 `Gy6k`。因此故障不是“没有配置 checker 专用 API”，也不是 Ideator 没有生成假设，而是 checker 没有复用正常 Agent 已经持有的运行时 LLM client。
...
这会触发第二套全局配置解析，通过 `settings.get_client()` 从进程环境和 `.env` 重新选取凭据。它与 runtime client 不具有同一性，也不保证使用相同 key。本次请求实际发送了日志所示的 `****63df`，随后被 DashScope 拒绝；正常 Agent 使用的 `****Gy6k` 没有到达 checker。
