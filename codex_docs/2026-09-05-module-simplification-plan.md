...
## Evaluator specification review

Read `spec.py` completely and traced every model field, validator, constant, loader option, helper, and export through trusted scoring, evaluator generation/freezing, experiment-document projection, Supervisor validation, and tests. Retain the strict/extra-allow schema boundary, relative-path and column validation, legacy/new contract distinction, both loaders, and the externally consumed default row-ID column. The `legacy_ok` flag has live permissive and strict callers.

Delete the unconsumed prediction-column constant, inline the two module-only file defaults into their fields, and delete the test-only `prediction_filename` helper. Route its path-injection contract through `EvaluatorSpec`, the actual production validation entrypoint. Reduce the explicit public surface from eight names to four and production code by 14 net lines without changing serialized fields, defaults, or validation policy.
...
## Baseline execution interface consolidation

Read `baseline.py` completely and traced authority load/seal/attestation, mirror restoration, ideator registration/two-turn repair, interruption/reap handling, Prepare Agent registration, frozen evaluator execution, and all direct callers. Preserve external compare-and-exchange authority, exact mirror matching, cached baseline reuse, one repair maximum, source verification before implementation, cancellation propagation, bounded cleanup, drift checks before/during/after scoring, and trusted completion attestation.
...
## Runtime facade surface follow-up

Full-file caller inventory found two redundant methods among the runtime facade's 79 methods. Delete `baseline_evaluator_ref`, which had no caller, and migrate the five test-only `supervisor_provider` reads to the existing production `provider` property; both provider properties returned the same lifecycle field. The facade falls from 720 to 710 physical lines and from 79 to 77 methods. The unchanged baseline and final provider/clarification/settings selection both passed 42 tests; Ruff, Black, old-symbol searches, Python compilation, hooks, and `git diff --check` passed.
...
## Experiment manifest review

Read `supervisor/manifest.py` completely and trace its schema, validation summary, workspace reader, experiment runner, validation guard, and direct unit/integration tests. Retain the module as the single strict boundary for Agent-authored `experiment.json`. Retain error-value redaction, bounded unknown-field names, argv validation, the Git executable ban, and absolute, drive-relative, empty-segment, and parent-segment path rejection because each protects an untrusted manifest boundary and has direct behavior coverage.
...
| `src/athena/research/prepare/baseline.py` | 629 | Reviewed; reduce both public entrypoints from six inputs to three and consolidate run resources |
